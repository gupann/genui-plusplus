...
function createEmptyEvaluations() {
  return ITERATION_3_MODELS.reduce((accumulator, model) => {
...
export default function Iteration3Study() {
  ...
  const [activeModelId, setActiveModelId] = useState(ITERATION_3_MODELS[0].id);
  ...
  useEffect(() => {
    ...
    async function hydrateSession() {
      ...
      if (loaded?.session) {
        ...
        setActiveModelId(snapshot.activeModelId || ITERATION_3_MODELS[0].id);
        setEvaluationsByModel(snapshot.evaluationsByModel || createEmptyEvaluations());
      } else {
        const snapshot = {
          phase: 'review',
          activeModelId: ITERATION_3_MODELS[0].id,
  ...
  }, [id, participantEmail, participantId]);
  ...
  const sessionSnapshot = useMemo(
    ...
    [phase, activeModelId, evaluationsByModel],
  );
  ...
  const availableModels = useMemo(
    () =>
      manifest.models.filter(
        (model) => outputsByModel[model.id]?.status === 'ready',
      ),
  ...
  const activeEvaluation =
    evaluationsByModel[activeModel.id] || createEmptyEvaluations()[activeModel.id];

  const allRequiredComplete = availableModels.every((model) => {
    const evaluation = evaluationsByModel[model.id];
    if (!evaluation?.overall) return false;
  ...
  function updateOverall(modelId, value) {
    setEvaluationsByModel((previous) => ({
      ...previous,
      [modelId]: {
        ...(previous[modelId] || {}),
  ...
  function updateComment(modelId, value) {
    setEvaluationsByModel((previous) => ({
      ...previous,
      [modelId]: {
        ...(previous[modelId] || {}),
    ...
    <div className='iteration3-study'>
      ...
      <section className='iteration3-study__compare-grid'>
        ...
        {manifest.models.map((model) => {
          const output = outputsByModel[model.id];
      ...
      <section className='iteration3-study__panel'>
        <div className='iteration3-study__tabs'>
          {manifest.models.map((model) => (
            <button
              key={model.id}
              type='button'
              className={`iteration3-study__tab${model.id === activeModel.id ? ' is-active' : ''}`}
        ...
        <div className='iteration3-study__feedback'>
          <div className='iteration3-study__feedback-header'>
            ...
            {outputsByModel[activeModel.id]?.status !== 'ready' && (
          ...
          <div className='iteration3-study__rubric-list'>
            {ITERATION_3_CRITERIA.map((criterion) => (
              <fieldset className='iteration3-study__criterion' key={criterion.id}>
                <legend>{criterion.label}</legend>
                <div className='iteration3-study__criterion-options'>
                  {ITERATION_3_RATING_OPTIONS.map((option) => (
                    <label key={option.value} className='iteration3-study__radio'>
                      <input
                        type='radio'
                        name={`${activeModel.id}-${criterion.id}`}
                        value={option.value}
                        checked={activeEvaluation.criteria?.[criterion.id] === option.value}
                        onChange={(event) =>
                          updateCriterion(activeModel.id, criterion.id, event.target.value)
                        }
                        disabled={outputsByModel[activeModel.id]?.status !== 'ready'}
                      />
          ...
          <div className='iteration3-study__overall'>
            <p className='iteration3-study__eyebrow'>C. Overall Evaluation</p>
              ...
              <div className='iteration3-study__overall-options'>
                {ITERATION_3_RATING_OPTIONS.map((option) => (
                  <label key={option.value} className='iteration3-study__radio'>
                    <input
                      type='radio'
                      name={`${activeModel.id}-overall`}
                      value={option.value}
                      checked={activeEvaluation.overall === option.value}
                      onChange={(event) =>
                        updateOverall(activeModel.id, event.target.value)
                      }
                      disabled={outputsByModel[activeModel.id]?.status !== 'ready'}
                    />
                    <span>{option.label}</span>
            ...
            <label className='iteration3-study__comment-label'>
              ...
              <textarea
                ...
                rows={4}
                value={activeEvaluation.comment || ''}
                onChange={(event) =>
                  updateComment(activeModel.id, event.target.value)
                }
...
const iteration3StudyStyles = `
  ...
  .iteration3-study__criterion legend,
  .iteration3-study__overall-group legend {
    padding: 0 0.25rem;
    font-size: 0.92rem;
    font-weight: 600;
  }
  .iteration3-study__criterion-options,
